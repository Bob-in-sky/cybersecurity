...
A shell is software that allows a user to interact with an <span style="color: inherit;">OS</span>. It can be a graphical interface, but it is usually a command-line interface, and this will depend on the operating system running on the target system.
...
- **<span style="color: inherit;">Persistence</span> and Maintenance Access**: Once shell access is obtained, attackers can create access through users and credentials or copy backdoor software to maintain access to the target system for later usage.
...
All of the shells we will describe in the next modules can help to achieve different limitations of the attacks described above.