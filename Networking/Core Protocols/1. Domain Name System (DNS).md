...
- **AAAA Record**: The AAAA record is similar to the A Record, but it is for IPv6. Remember that it is AAAA (quad-A), as AA and AAA would refer to a battery size; furthermore, AAA refers to *Authentication, Authorization, and Accounting*; neither falls under <span style="color: inherit;">DNS</span>.
...
```shell
bobinsson@BoB$ nslookup www.example.com
Server:         127.0.0.53
...
```shell
bobinsson@BoB$ tshark -r dns-query.pcapng -Nn
